...
#      with the sandbox).
#   3. Upload ../app/server.py and start it on :5000 with the env vars
...
#   7. Add port 5000 via data-plane ports/add with
#      entraId.objectIds=[gateway MI] (+ tenantIds + your email so you
#      can hit /healthz from a browser).
...
CONFIG_NAME="email-to-sandbox-demo"
...
[[ -f "$dir/.env" ]] || { echo "error: samples/.env not found - run setup first" >&2; exit 1; }
set -a; . "$dir/.env"; set +a
...
for v in "${REQUIRED[@]}"; do
    if [[ -z "${!v:-}" ]]; then
        echo "error: missing env var '$v' in samples/.env." >&2
        echo "       Run, in order:" >&2
        echo "         ../../../sandboxes/setup/cli/setup.sh" >&2
        echo "         ../../setup/cli/setup.sh" >&2
        exit 1
    fi
done
...
# python3 here is just for URL parsing; the sandbox uses its own python3.
host_from_url() {
    python3 - <<EOF "$1"
...
cleanup() {
    ...
    if [[ -n "$SANDBOX_ID" && "$PORT_ADDED" == "1" ]]; then
        ...
        az rest --method POST \
            ...
            --resource "$DATAPLANE_RESOURCE" --body "@$body" \
...
# matching SG-level gatewayConnections entry (PATCHed by setup.sh)
# provides the runtime URL + auth type; we only need to pass the
# resourceId here.
...
SANDBOX_BODY="$(mktmp)"
python3 - "$CONNECTION_RESOURCE_ID" "$RUN_ID" > "$SANDBOX_BODY" <<'PYEOF'
import json, sys
...
CREATE_RESP="$(az rest --method PUT \
    ...
    --body "@$SANDBOX_BODY")"
SANDBOX_ID="$(python3 - "$CREATE_RESP" <<'PYEOF'
import json, sys
try:
...
# ----- 3. Upload + start -------------------------------------------------
echo "==> Uploading app/server.py into /app..."
...
# ----- 6. Port (data plane POST /ports/add with entraId.objectIds) ------
echo "==> add port $PORT (entraId.objectIds=[gateway MI]${GW_TENANT:+, tenantIds=[gateway tenant]})"
PORT_BODY="$(mktmp)"
python3 - "$PORT" "$GW_PRINCIPAL" "$GW_TENANT" "$USER_EMAIL" > "$PORT_BODY" <<'PYEOF'
import json, sys
port = int(sys.argv[1])
...
print(json.dumps({"port": port, "auth": {"entraId": entra}}))
PYEOF
...
PORT_URL="$(python3 - "$PORT" "$PORT_RESP" <<'PYEOF'
import json, sys
port = int(sys.argv[1])
data = json.loads(sys.argv[2] or "{}")
ports = data.get("ports") or []
match = next((p for p in ports if isinstance(p, dict) and p.get("port") == port), None)
print((match or {}).get("url", "") or data.get("url", ""))
PYEOF
)"
[[ -n "$PORT_URL" ]] || { echo "error: ports/add returned no url" >&2; echo "$PORT_RESP" >&2; exit 1; }
...
python3 - "$CONN" "$SG" "$SANDBOX_ID" "$CALLBACK_URL" "$SUBJECT_FILTER" > "$BODY_FILE" <<'PYEOF'
import json, sys
...
echo
echo "========================================================================"
echo "Email-to-sandbox trigger is live"
echo "========================================================================"
