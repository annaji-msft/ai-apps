...
Creates (all idempotent):
  ...
  1. A connector gateway with SystemAssigned managed identity, in the
     resource group provisioned by the sandboxes pillar's setup.
...
Then appends the gateway/connection keys to ``samples/.env`` so the
...
Prerequisites:
  * Sandboxes baseline has been provisioned
...
def _load_env() -> dict[str, str]:
    """Load samples/.env into both os.environ and a dict for inspection."""
    if not ENV_FILE.exists():
        sys.exit(
            "error: samples/.env does not exist. Run the sandboxes pillar\n"
            "       baseline first:\n"
    ...
    if missing:
        sys.exit(
            f"error: samples/.env is missing {missing}. Run the sandboxes\n"
            "       pillar baseline first."
        )
    return values
...
def _ensure_sandbox_group_identity(rg: str, sg: str) -> str:
    ...
    The sandboxes-pillar baseline creates the group with no identity. The
    send-side access policy needs the sandbox-group MI as a principal, so
    this script ensures it exists and surfaces the principalId for callers
    to write into ``samples/.env``.
    ...
    in ``samples/.env``). ``rg`` is accepted only so error messages can
    refer to it.
    """
    def _read_principal() -> str:
        ...
        # already in samples/.env). Returns exit code != 0 if the group
...
def _write_env_file(values: dict[str, str]) -> None:
    ...
    lines = [
        "# Updated by samples/sandboxes/scenarios/10-connectors-triggers/setup/python/setup.py",
        "# Re-run sandbox or connector-trigger setup to update.",
        "",
    ]
...
def main() -> None:
    ...
    # colocated. The sandboxes pillar writes both ACA_SANDBOXGROUP_REGION
    # (canonical) and ACA_REGION (legacy alias) into samples/.env.
    region = (
    ...
    if not region:
        sys.exit(
            ...
            "ACA_CONNECTOR_GATEWAY_REGION explicitly, or run the sandboxes "
            "pillar baseline first (it writes ACA_SANDBOXGROUP_REGION into "
            "samples/.env)."
        )
    ...
    if status != "Connected":
        ...
        if args.non_interactive:
            print()
            print("--non-interactive set; not opening browser or waiting.")
            print("Complete consent above, then re-run this script.")
            sys.exit(2)
        try:
    ...
    print(f"==> Ensuring sandbox-group '{env.get('ACA_SANDBOX_GROUP', '?')}' has SystemAssigned identity...")
    sandbox_group_name = env.get("ACA_SANDBOX_GROUP") or os.environ.get("ACA_SANDBOX_GROUP")
    if not sandbox_group_name:
        sys.exit(
            "error: ACA_SANDBOX_GROUP missing from samples/.env. Run the\n"
            "       sandboxes pillar baseline first."
        )
    ...
    print("==> Done.")
    print()
    print("Next:  cd ../../trigger-lifecycle/python && python trigger.py")
