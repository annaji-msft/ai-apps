...
This script does NOT touch the sandboxes baseline (resource group or
sandbox group itself) — use ``samples/sandboxes/setup/python/teardown.py``
for that.
...
def _load_env() -> None:
    if not ENV_FILE.exists():
        sys.exit("error: samples/.env not found - nothing to tear down.")
    ...
    for line in ENV_FILE.read_text().splitlines():
        if line and not line.startswith("#") and "=" in line:
            ...
            if k not in keys:
                kept[k] = v
    lines = [
        "# Updated by samples/sandboxes/scenarios/10-connectors-triggers/setup/python/teardown.py",
...
def main() -> None:
    ...
    if not (subscription_id and resource_group and gateway):
        sys.exit(
            "error: samples/.env missing trigger keys - was connector-gateway setup run?"
        )
    ...
    print("  trigger-related keys from samples/.env")
    print()
