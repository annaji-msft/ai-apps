...
| 10 | [connectors-email-triage](10-connectors-email-triage) | **Azure Connector Gateway** trigger fires on new email → ACA receiver boots a per-email sandbox → Copilot CLI in the sandbox posts a triage card to Teams via the gateway's **Managed MCP server**. The egress proxy stamps the gateway API key onto outbound MCP requests so the sandbox holds no credential. Ships full `azd up` (Bicep + Container App + post-deploy script for OAuth consent). | ✅ azd-deployable |
