...
| 05 | [data-processing](scenarios/05-data-processing) | Producer/consumer pipeline on a shared AzureBlob volume. Producer streams synthetic events into `/mnt/shared/raw/`, transformer enriches them into `/mnt/shared/processed/` concurrently, aggregator summarises. Pure-stdlib workers (no Blob SDK). | ✅ Python ready · 📝 CLI planned |
| 06 | [developer-workflows](scenarios/06-developer-workflows) | Ephemeral CI runner — cold-boot a base sandbox, install toolchain, snapshot, then build N PRs in parallel from the snapshot (warm starts). Ships 3 synthetic PRs (2 pass, 1 deliberate regression) that verify the failure path. | ✅ Python ready |
